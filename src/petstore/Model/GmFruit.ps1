...
#>
function ConvertFrom-JsonToGmFruit {
    [CmdletBinding()]
    Param (
        [AllowEmptyString()]
        [string]$Json
    ...
    Process {
        ...
        if ($match -ne 0) { # no match yet
            ...
            try {
                $matchInstance = ConvertFrom-JsonToApple $Json
                ...
                Write-Debug "Failed to match 'Apple' defined in anyOf (GmFruit). Proceeding to the next one if any."
            }
        }
        ...
        if ($match -ne 0) { # no match yet
            ...
            try {
                $matchInstance = ConvertFrom-JsonToBanana $Json
                ...
                # fail to match the schema defined in anyOf, proceed to the next one
                Write-Debug "Failed to match 'Banana' defined in anyOf (GmFruit). Proceeding to the next one if any."
            }
        }
